/* This is the user setting config file.
 *  You need to change some of these 
 *  values in order for the device to 
 *  work as desired.
 *  
 *  Changing the SSID and Wifi password
 *  are mandatory.                       */
#define STASSID "<SSID>"
#define STAPSK  "<WIFI PASSWORD>"

// Set stealthMode off. Turns off all LEDs and LED info.
bool stealthMode = 0;

// Set useDHT 
bool useDHT = 0;
#define DHTPIN 3
#define DHTTYPE DHT22

// Set useMQ sensors off. Not using any MQ gas sensors
bool useMQ = 0;

/* Set useMasterServer off. You probably want to use this. 
 *  Turned off by default untill you edit server settings. */
bool useMaster = 0;
// Server ssl cert fingerprint. Change this to your servers certificate fingerprint
const uint8_t fingerprint[20] = {0x5A, 0xCF, 0xFE, 0xF0, 0xF1, 0xA6, 0xF4, 0x5F, 0xD2, 0x11, 0x11, 0xC6, 0x1D, 0x2F, 0x0E, 0xBC, 0x39, 0x8D, 0x50, 0xE0};
// Server address
String serverURL = "https://jigsaw.w3.org/HTTP/connection.html";

/* AES256 key: Change this. 
 *  Use an online text to hex converter to do this.
 *  Example: https://www.online-toolz.com/tools/text-hex-convertor.php
 *  You may need to manually add the 0x to each hex number.
 */
  uint8_t key[] = { //
    0x60, 0x67, 0x36, 0x30, 0x47, 0x7b, 0x26, 0x6c, 
    0x40, 0x6d, 0x4f, 0x4a, 0x2c, 0x43, 0x28, 0x24, 
    0x2e, 0x55, 0x23, 0x70, 0x26, 0x5e, 0x78, 0x4c, 
    0x5a, 0x3b, 0x42, 0x36, 0x23, 0x49
  };

// Change these to real certs. The ones below are autogenerated and part of a tutorial.

static const char serverCert[] PROGMEM = R"EOF(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
)EOF";

static const char serverKey[] PROGMEM =  R"EOF(
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
)EOF";
// DO NOT EDIT BELOW
const char* ssid = STASSID;
const char* password = STAPSK;
DHT_Unified dht(DHTPIN, DHTTYPE);
